<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Programmer une séance') }}
        </h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <p>Remplissez le formulaire ci-dessous pour ajouter une séance :</p>

                    <form action="/update-forfait" method="post">
                    
                        {{ csrf_field() }}

                        <input type="text" name="nomForfait" placeholer="Nom"  value="{{ $forfait->nom }}" required> 
                        <textarea name="modalitesForfait" placeholder="Modalités" required>{{ $forfait->modalites }}</textarea>
                        <input type="number" name="prixForfait" placeholder="Prix" step="any"  value="{{ $forfait->prix }}" required>
                        <label for="prix-forfait">€</label>

                        <input type="hidden" name="id" value="{{ $forfait->id_forfait }}" >
                        <input type="submit" value="Enregistrer" >

                    </form>
                </div>
            </div>

        </div>
    </div>
</x-app-layout>
